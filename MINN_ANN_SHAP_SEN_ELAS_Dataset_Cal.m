...
%% ---------------- LOAD DATA ----------------
Ttr = readtable(trainFile,'VariableNamingRule','preserve');
Tte = readtable(testFile ,'VariableNamingRule','preserve');

Ttr.Properties.VariableNames = matlab.lang.makeValidName( ...
    strtrim(Ttr.Properties.VariableNames));

Tte.Properties.VariableNames = matlab.lang.makeValidName( ...
    strtrim(Tte.Properties.VariableNames));
...
Y_ANN_train = predictModel_vec( ...
    Xtrain, ANN.net, ANN.muX(:), ANN.stdX(:), ...
    ANN.muy, ANN.stdy);

Y_MINN_train = predictModel_vec( ...
    Xtrain, MINN.net, MINN.muX(:), MINN.stdX(:), ...
    MINN.muy, MINN.stdy);

Y_ANN_test = predictModel_vec( ...
    Xtest, ANN.net, ANN.muX(:), ANN.stdX(:), ...
    ANN.muy, ANN.stdy);

Y_MINN_test = predictModel_vec( ...
    Xtest, MINN.net, MINN.muX(:), MINN.stdX(:), ...
    MINN.muy, MINN.stdy);
...
[E_ANN_train, dY_ANN_train, pct_ANN_train] = calculateElasticityForward( ...
    Xtrain, ANN.net, ANN.muX(:), ANN.stdX(:), ...
    ANN.muy, ANN.stdy, relStep_default);

fprintf('\nCalculating ANN Test sensitivity and elasticity...\n');

[E_ANN_test, dY_ANN_test, pct_ANN_test] = calculateElasticityForward( ...
    Xtest, ANN.net, ANN.muX(:), ANN.stdX(:), ...
    ANN.muy, ANN.stdy, relStep_default);

fprintf('\nCalculating MINN Train sensitivity and elasticity...\n');

[E_MINN_train, dY_MINN_train, pct_MINN_train] = calculateElasticityForward( ...
    Xtrain, MINN.net, MINN.muX(:), MINN.stdX(:), ...
    MINN.muy, MINN.stdy, relStep_default);

fprintf('\nCalculating MINN Test sensitivity and elasticity...\n');

[E_MINN_test, dY_MINN_test, pct_MINN_test] = calculateElasticityForward( ...
    Xtest, MINN.net, MINN.muX(:), MINN.stdX(:), ...
    MINN.muy, MINN.stdy, relStep_default);
...
SHAP_ANN_train = calculatePermutationSHAP( ...
    Xtrain, Xtrain, ...
    ANN.net, ANN.muX(:), ANN.stdX(:), ...
    ANN.muy, ANN.stdy, nPermSHAP);

fprintf('\nCalculating ANN SHAP for Test data...\n');

SHAP_ANN_test = calculatePermutationSHAP( ...
    Xtest, Xtrain, ...
    ANN.net, ANN.muX(:), ANN.stdX(:), ...
    ANN.muy, ANN.stdy, nPermSHAP);
...
SHAP_MINN_train = calculatePermutationSHAP( ...
    Xtrain, Xtrain, ...
    MINN.net, MINN.muX(:), MINN.stdX(:), ...
    MINN.muy, MINN.stdy, nPermSHAP);

fprintf('\nCalculating MINN SHAP for Test data...\n');

SHAP_MINN_test = calculatePermutationSHAP( ...
    Xtest, Xtrain, ...
    MINN.net, MINN.muX(:), MINN.stdX(:), ...
    MINN.muy, MINN.stdy, nPermSHAP);
