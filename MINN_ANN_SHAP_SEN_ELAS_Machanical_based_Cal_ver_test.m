...
%   - ANN_N_S_BayesianOptimization_5Fold_Results.mat
%   - Results_MINN_MOBO_5Fold/MINN_MOBO_5Fold_Results.mat
...
%% ---------------- USER SETTINGS ----------------
% Trained 5-fold ANN ensemble exported by the Bayesian optimization script.
% No ANN retraining is performed in this file.
annModelFile  = 'ANN_N_S_BayesianOptimization_5Fold_Results.mat';
% Trained 5-fold MINN ensemble exported by the MOBO script.
% No MINN retraining is performed in this file.
minnModelFile = fullfile('Results_MINN_MOBO_5Fold', ...
    'MINN_MOBO_5Fold_Results.mat');
...
% Permutation feature importance (PFI): increase in RMSE after shuffling
% one feature at a time.  Positive = feature improves prediction accuracy.
nPermPFI = 100;
...
%% ---------------- LOAD MODELS ----------------
ANN = load(annModelFile,'bestFoldModels','bestFoldMuX','bestFoldStdX', ...
    'bestFoldMuy','bestFoldStdy','bestHiddenLayers', ...
    'bestNumNeurons','bestLearningRate','testMetrics');
MINN = load(minnModelFile,'bestModels','bestNorm','bestX','testMetrics');
...
if isfield(ANN,'bestHiddenLayers')
    fprintf(['ANN best parameters: HL=%d | Neurons=%d | ' ...
        'LR=%.17g\n'],ANN.bestHiddenLayers,ANN.bestNumNeurons, ...
        ANN.bestLearningRate);
end
fprintf('Loaded trained MINN ensemble = %d fold models\n', numel(MINN.bestModels));
if isfield(MINN,'bestX')
    fprintf(['MINN best parameters: HL=%d | Neurons=%d | ' ...
        'LR=%.17g | lambda_d=%.17g\n'], ...
        MINN.bestX.NumHiddenLayers, MINN.bestX.NumNeurons, ...
        10.^MINN.bestX.Log10LearningRate, ...
        10.^MINN.bestX.Log10LambdaData);
end
...
% Experimental capacity is required for PFI (PFI measures loss increase).
if any(strcmp(Tte.Properties.VariableNames,'N'))
    Yexp_test = Tte.N;
elseif any(strcmp(Tte.Properties.VariableNames,'Nu'))
    Yexp_test = Tte.Nu;
else
    error('Missing experimental capacity column N or Nu required for PFI.');
end
Yexp_test = double(Yexp_test(:));
...
%% ============================================================
% STEP 4B: PERMUTATION FEATURE IMPORTANCE (PFI)
% Positive PFI means that randomising the feature increases RMSE and hence
% that feature is important. A negative value can occur by sampling noise.
%% ============================================================
fprintf('\nCalculating ANN permutation feature importance...\n');
PFI_ANN = calculatePFI_ANNEnsemble(Xtest, Yexp_test, ANN, nPermPFI);

fprintf('\nCalculating MINN permutation feature importance...\n');
PFI_MINN = calculatePFI_MINNEnsemble(Xtest, Yexp_test, ...
    MINN.bestModels, MINN.bestNorm, nPermPFI);
...
All_PFI_Summary = [ ...
    createPFISummary(featShow, PFI_ANN,  'ANN',  'Test');
    createPFISummary(featShow, PFI_MINN, 'MINN', 'Test')
];
...
% Waterfall-style PFI bars: blue = positive importance; pink = negative.
plotPFIWaterfall(PFI_ANN, featShow, 'ANN', ...
    fullfile(outDir,'PFI_ANN_Waterfall.png'), fontName, outDPI, doExportFigure);
plotPFIWaterfall(PFI_MINN, featShow, 'MINN', ...
    fullfile(outDir,'PFI_MINN_Waterfall.png'), fontName, outDPI, doExportFigure);
...
function checkANNEnsembleFields(S)
    ...
    for k = 1:numel(requiredFields)
        if ~isfield(S,requiredFields{k})
            error(['ANN ensemble file missing variable "%s". Use the ' ...
                'Results MAT file exported by ANN Bayesian Optimization.'], ...
                requiredFields{k});
        end
    end
    K = numel(S.bestFoldModels);
    if ~iscell(S.bestFoldModels) || K == 0 || ...
...
function checkMINNEnsembleFields(S)
    ...
    for k = 1:numel(requiredFields)
        if ~isfield(S,requiredFields{k})
            error(['MINN ensemble file missing variable "%s". ' ...
                'Use MINN_MOBO_5Fold_Results.mat exported after MOBO.'], ...
                requiredFields{k});
        end
    end
    ...
    if ~iscell(S.bestModels) || ~iscell(S.bestNorm) || ...
        ...
        error('bestModels and bestNorm must be nonempty cell arrays of equal length.');
    end
...
function PFI = calculatePFI_ANNEnsemble(X, Yexp, ANN, nPerm)
    yBase = predictANNEnsemble_vec(X, ANN);
    baseRMSE = sqrt(mean((Yexp - yBase).^2, 'omitnan'));
    nFeat = size(X,2);
    rmseIncrease = nan(nPerm,nFeat);

    for p = 1:nPerm
        for j = 1:nFeat
            Xperm = X;
            Xperm(:,j) = X(randperm(size(X,1)),j);
            yPerm = predictANNEnsemble_vec(Xperm, ANN);
            permRMSE = sqrt(mean((Yexp - yPerm).^2, 'omitnan'));
            rmseIncrease(p,j) = permRMSE - baseRMSE;
        end
    end
    PFI = struct('Baseline_RMSE',baseRMSE, ...
        'RMSE_Increase',rmseIncrease, ...
        'Mean_RMSE_Increase',mean(rmseIncrease,1,'omitnan'), ...
        'Std_RMSE_Increase',std(rmseIncrease,0,1,'omitnan'));
end

function PFI = calculatePFI_MINNEnsemble(X, Yexp, models, norms, nPerm)
    yBase = predictMINNEnsemble_vec(X, models, norms);
    baseRMSE = sqrt(mean((Yexp - yBase).^2, 'omitnan'));
    nFeat = size(X,2);
    rmseIncrease = nan(nPerm,nFeat);
    ...
    for p = 1:nPerm
        for j = 1:nFeat
            Xperm = X;
            Xperm(:,j) = X(randperm(size(X,1)),j);
            yPerm = predictMINNEnsemble_vec(Xperm, models, norms);
            permRMSE = sqrt(mean((Yexp - yPerm).^2, 'omitnan'));
            rmseIncrease(p,j) = permRMSE - baseRMSE;
        end
    end
...
function Tpfi = createPFISummary(featShow, PFI, modelName, dataName)
    meanIncrease = PFI.Mean_RMSE_Increase(:);
    totalPositive = sum(max(meanIncrease,0),'omitnan');
    if totalPositive > 0
        normalizedPFI = max(meanIncrease,0) ./ totalPositive * 100;
    else
        normalizedPFI = nan(size(meanIncrease));
    end
    Tpfi = table( ...
        repmat({modelName},numel(featShow),1), repmat({dataName},numel(featShow),1), ...
        featShow(:), repmat(PFI.Baseline_RMSE,numel(featShow),1), ...
        meanIncrease, PFI.Std_RMSE_Increase(:), normalizedPFI, ...
        'VariableNames',{'Model','Data','Variable','Baseline_RMSE', ...
        'Mean_RMSE_Increase','Std_RMSE_Increase','Normalized_PFI_percent'});
    Tpfi.PFI_Rank = tiedrank(-Tpfi.Mean_RMSE_Increase);
    Tpfi = sortrows(Tpfi,{'Model','Data','PFI_Rank'});
end
...
function plotPFIWaterfall(PFI, featShow, modelName, outName, fontName, outDPI, doExportFigure)
    % Horizontal PFI figure styled like a SHAP waterfall:
    % blue bars increase RMSE when permuted (important); pink bars decrease it.
    values = PFI.Mean_RMSE_Increase(:);
    errors = PFI.Std_RMSE_Increase(:);
    ...
    blue = [0.00 0.55 0.90];
    pink = [0.93 0.02 0.35];
    barColors = repmat(blue,n,1);
    barColors(values < 0,:) = repmat(pink,sum(values < 0),1);

    fig = figure('Color','w','Position',[120 120 950 560]);
    ax = axes(fig); hold(ax,'on'); box(ax,'off');
    b = barh(ax,1:n,values,0.70,'FaceColor','flat','EdgeColor','none');
    b.CData = barColors;
    errorbar(ax,values,1:n,errors,'horizontal','k','LineStyle','none', ...
        'LineWidth',1.0,'CapSize',7);
    xline(ax,0,':','Color',[0.35 0.35 0.35],'LineWidth',1.1);

    xlabel(ax,'Increase in RMSE after permutation (kN)', ...
        'FontName',fontName,'FontSize',17);
    yticks(ax,1:n); yticklabels(ax,labels);
    set(ax,'YDir','reverse','FontName',fontName,'FontSize',16, ...
        'LineWidth',1.1,'TickLabelInterpreter','none');
    title(ax,sprintf('%s permutation feature importance',modelName), ...
        'FontName',fontName,'FontSize',19,'FontWeight','bold');

    span = max(values + errors) - min(values - errors);
    if ~isfinite(span) || span <= 0, span = 1; end
    xPad = 0.025*span;
    for k = 1:n
        if values(k) >= 0
            xText = values(k) + errors(k) + xPad;
            alignment = 'left';
        else
            xText = values(k) - errors(k) - xPad;
            alignment = 'right';
        end
        text(ax,xText,k,sprintf('%+.1f',values(k)), ...
            'HorizontalAlignment',alignment,'VerticalAlignment','middle', ...
            'FontName',fontName,'FontSize',14,'FontWeight','bold', ...
            'Color',barColors(k,:));
    end

    xlim(ax,[min([values-errors;0])-0.12*span, ...
             max([values+errors;0])+0.18*span]);
    annotation(fig,'textbox',[0.68 0.84 0.26 0.09], ...
        'String',{'Blue: RMSE increases','Pink: RMSE decreases'}, ...
        'FontName',fontName,'FontSize',12,'EdgeColor',[0.7 0.7 0.7], ...
